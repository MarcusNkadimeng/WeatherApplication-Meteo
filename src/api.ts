...
import { convertToCurrentWeather } from "./util";

export async function getCurrentWeather(): Promise<currentWeather> {
  try {
    const currentWeatherRes = await fetch(
      "https://api.open-meteo.com/v1/forecast?latitude=-25.7857014&longitude=28.280809&current=temperature_2m,apparent_temperature,weather_code,wind_speed_10m,wind_direction_10m&timezone=auto"
    );
...
export async function get7DayForecast(): Promise<DailyWeather[]> {
  try {
    const dailyWeatherRes = await fetch(
      "https://api.open-meteo.com/v1/forecast?latitude=-25.7857014&longitude=28.280809&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=auto"
    );
